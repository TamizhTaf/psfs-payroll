...
public class JwtRequestFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
	    ...
	    // Skip the JWT validation for these public endpoints
	    if ("/liveness".equalsIgnoreCase(servletPath) || 
	        "/api/auth/register".equalsIgnoreCase(servletPath) ||
	        "/api/auth/signin".equalsIgnoreCase(servletPath) ||
	        "/api/auth/signout".equalsIgnoreCase(servletPath)) {
	        
	        chain.doFilter(request, response);
	        return; // Return immediately to avoid unnecessary processing
	    }

	    // Otherwise, handle token validation for other endpoints
	    ...
	    if (!ServiceUtil.isNotEmpty(jwtToken)) {
	        ...
	        if (ServiceUtil.isNotEmpty(username)) {
	            ...
	            if (userDetails == null) {
	                validTokenMap.put("status", "failed");
	                validTokenMap.put("message", "User not logged in!");
	                writeErrorDetail(response, validTokenMap);
	                return;
